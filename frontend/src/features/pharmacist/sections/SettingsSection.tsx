...
function buildInitials(name: string | null | undefined) {
  const safe = (name || "Pharmacist").trim();
...
export function SettingsSection({
  ...
  const previewName = preferredName.trim() || user?.preferredName || user?.name || "Pharmacist";
  const previewInitials = buildInitials(previewName);
  const organisationId = pharmacyId || (user?.organisationId != null ? String(user.organisationId) : null);
  const storageKey = useMemo(
    () => `pharmacist-profile-photo:${user?.id ?? user?.email ?? "guest"}`,
    [user?.email, user?.id],
  );
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-3xl border border-slate-200 bg-slate-50 text-2xl font-black text-cyan-700 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-blue-300">
              {photo ? <img src={photo} alt={`${previewName} avatar`} className="h-full w-full object-cover" /> : previewInitials}
            </div>
            <div>
              ...
              <h2 className="mt-3 text-3xl font-black text-slate-900 dark:text-white">{previewName}</h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                {pharmacyName ?? user?.organisationName ?? "Affiliated pharmacy not linked"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="rounded-[2rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(224,242,254,0.7),rgba(186,230,253,0.4))] p-8 shadow-sm backdrop-blur-sm dark:border-slate-700/70 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.6),rgba(8,10,15,0.8))]">
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <label className="block">
              <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-slate-400">Preferred Name</span>
              <input
                type="text"
                value={preferredName}
                ...
                placeholder="Dispensing Pharmacist"
              />
            </label>
            <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-300">
              <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-slate-400">Legal name on NIC</p>
              <p className="mt-2 font-semibold">{legalName}</p>
            </div>
            <label className="block md:col-span-2">
              <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-slate-400">Address</span>
              <textarea
                value={address}
          ...
          {saveMessage ? (
            <div className={`mt-6 rounded-2xl border px-4 py-3 text-sm ${noticeClassName(saveMessage)}`}>{saveMessage}</div>
          ) : null}
        </div>
          ...
          <div className="rounded-[2rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(224,242,254,0.7),rgba(186,230,253,0.4))] p-8 shadow-sm backdrop-blur-sm dark:border-slate-700/70 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.6),rgba(8,10,15,0.8))]">
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700">
              <div className="border-b border-slate-100 bg-slate-50 px-5 py-4 dark:border-slate-800 dark:bg-slate-800/40">
                ...
                <p className="mt-2 text-xl font-black text-slate-900 dark:text-white">{user?.email ?? "No email saved"}</p>
              </div>
              <div className="border-b border-slate-100 bg-white px-5 py-4 dark:border-slate-800 dark:bg-slate-900">
                <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">License Number</p>
                <p className="mt-2 text-xl font-black text-blue-700 dark:text-blue-300">{user?.licenseNumber ?? "Unavailable"}</p>
              </div>
              <div className="border-b border-slate-100 bg-slate-50 px-5 py-4 dark:border-slate-800 dark:bg-slate-800/40">
                <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Pharmacy</p>
                <p className="mt-2 text-xl font-black text-slate-900 dark:text-white">{pharmacyName ?? user?.organisationName ?? "Not linked"}</p>
              </div>
              <div className="border-b border-slate-100 bg-white px-5 py-4 dark:border-slate-800 dark:bg-slate-900">
                <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Organisation ID</p>
                <p className="mt-2 text-lg font-black text-slate-900 dark:text-white">{organisationId ?? "Not linked"}</p>
              </div>
              <div className="border-b border-slate-100 bg-slate-50 px-5 py-4 dark:border-slate-800 dark:bg-slate-800/40">
                <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Account Status</p>
                <p className="mt-2 text-lg font-black text-slate-900 dark:text-white">{formatStatusLabel(user?.status ?? user?.organisationStatus)}</p>
              </div>
              <div className="bg-white px-5 py-4 dark:bg-slate-900">
                <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Dispense Events Today</p>
                <p className="mt-2 text-lg font-black text-slate-900 dark:text-white">{dispensedToday}</p>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-[2rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(224,242,254,0.7),rgba(186,230,253,0.4))] p-8 shadow-sm backdrop-blur-sm dark:border-slate-700/70 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.6),rgba(8,10,15,0.8))]">
            ...
            <div className="mt-6 space-y-4">
              <div className="rounded-2xl border border-slate-200 bg-white px-5 py-4 dark:border-slate-700 dark:bg-slate-900">
                <p className="text-[10px] font-bold uppercase tracking-[0.24em] text-slate-400">Recorded history rows</p>
                <p className="mt-2 text-base font-bold text-slate-900 dark:text-white">{historyCount}</p>
              </div>
            </div>
          </div>
        </div>
