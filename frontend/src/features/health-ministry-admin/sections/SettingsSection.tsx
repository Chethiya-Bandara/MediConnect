...
export function SettingsSection({
  ...
  const previewName = preferredName.trim() || user?.preferredName || user?.name || "Ministry Admin";
  const previewInitials = buildInitials(previewName);
  const storageKey = useMemo(
    () => `moh-admin-profile-photo:${user?.id ?? user?.email ?? "guest"}`,
    [user?.email, user?.id],
  );
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <p className="rounded-full border border-cyan-200 bg-cyan-50 px-4 py-1 text-[10px] font-black uppercase tracking-[0.3em] text-cyan-700 dark:border-blue-900/40 dark:bg-blue-900/20 dark:text-blue-300">
                Ministry Admin Profile
              </p>
              <h2 className="mt-3 text-3xl font-black text-slate-900 dark:text-white">{previewName}</h2>
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">National control workspace</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-700 dark:bg-slate-800">
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Update the ministry-facing identity shown across the national control console.</p>
            </div>
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">Upload a photo or stick with the initials badge.</p>
                </div>
              </div>
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                placeholder="Ministry Control Lead"
              />
            </label>
          ...
          <div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-700 dark:bg-slate-800">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                ...
                <h3 className="mt-2 text-2xl font-extrabold text-slate-900 dark:text-white">Ministry scope</h3>
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Verified national-control context tied to this admin account.</p>
              </div>
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-cyan-200 bg-cyan-50 text-cyan-700 shadow-sm dark:border-blue-900/40 dark:bg-blue-900/20 dark:text-blue-300">
                <ShieldCheck size={22} />
              </div>
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700">
              ...
              <div className="border-b border-slate-100 bg-white px-5 py-4 dark:border-slate-800 dark:bg-slate-900">
                <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Role</p>
                <p className="mt-2 text-xl font-black text-blue-700 dark:text-blue-300">{formatStatusLabel(user?.adminRole ?? user?.role)}</p>
              </div>
              <div className="border-b border-slate-100 bg-slate-50 px-5 py-4 dark:border-slate-800 dark:bg-slate-800/40">
                <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Account Status</p>
                <p className="mt-2 text-xl font-black text-slate-900 dark:text-white">{formatStatusLabel(user?.status)}</p>
              </div>
            </div>
          </div>
        </div>
