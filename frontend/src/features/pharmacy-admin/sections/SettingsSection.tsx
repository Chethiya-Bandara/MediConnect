...
function buildInitials(name: string | null | undefined) {
  const safe = (name || "Pharmacy Admin").trim();
...
export function SettingsSection({
  ...
  const previewName = preferredName.trim() || user?.preferredName || user?.name || "Pharmacy Admin";
  const previewInitials = buildInitials(previewName);
  const organisationId =
    activePharmacyId ?? (user?.organisationId != null ? String(user.organisationId) : null);
  const storageKey = useMemo(
    () => `pharmacy-admin-profile-photo:${user?.id ?? user?.email ?? "guest"}`,
    [user?.email, user?.id],
  );
  ...
  const handlePhotoSelected = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      if (typeof window !== "undefined") {
        if (result) window.localStorage.setItem(storageKey, result);
      }
    };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
                {user?.organisationName ?? `Organisation #${activePharmacyId ?? "N/A"}`}
              </p>
            </div>
          </div>
          <button
          ...
          <div className="mb-6 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                Update the profile details tied to this pharmacy operations account.
              </p>
            </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm text-slate-600 dark:text-slate-300">
                    Upload a photo or rock the initials bubble.
                  </p>
                </div>
              </div>
          ...
          <div className="mt-6 grid gap-5 md:grid-cols-2">
            <label className="block">
              ...
              <input
                ...
                placeholder="Lead Pharmacy Admin"
              />
            </label>
          ...
          <div className="rounded-[2rem] border border-slate-200/80 bg-[linear-gradient(180deg,rgba(224,242,254,0.7),rgba(186,230,253,0.4))] p-8 shadow-sm backdrop-blur-sm dark:border-slate-700/70 dark:bg-[linear-gradient(180deg,rgba(15,23,42,0.6),rgba(8,10,15,0.8))]">
            <div className="mb-6 flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
                  Live operational context for this admin account.
                </p>
              </div>
              <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-cyan-200 bg-cyan-50 text-cyan-700 shadow-sm dark:border-blue-900/40 dark:bg-blue-900/20 dark:text-blue-300">
                <ShieldCheck size={22} />
            ...
            <div className="overflow-hidden rounded-3xl border border-slate-200 dark:border-slate-700">
              ...
              <div className="border-b border-slate-100 bg-white px-5 py-4 dark:border-slate-800 dark:bg-slate-900">
                <p className="text-[10px] font-bold uppercase tracking-[0.35em] text-slate-400">Organisation</p>
                <p className="mt-2 text-xl font-black text-blue-700 dark:text-blue-300">
                  {user?.organisationName ?? `Pharmacy Org #${activePharmacyId ?? "N/A"}`}
                </p>
              </div>
              <div className="border-b border-slate-100 bg-slate-50 px-5 py-4 dark:border-slate-800 dark:bg-slate-800/40">
              ...
              <div className="border-b border-slate-100 bg-slate-50 px-5 py-4 dark:border-slate-800 dark:bg-slate-800/40">
                ...
                <p className="mt-2 text-xl font-black text-slate-900 dark:text-white">
                  {formatStatusLabel(user?.status ?? user?.organisationStatus)}
                </p>
              </div>
            </div>
          </div>
